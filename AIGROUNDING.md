...
## Session History
- **2025-11-04**: Landed the end-to-end catalog pipeline. Materialized the ERD into SQLModel tables with FastAPI exposure (`/catalog/activities`, `/catalog/search`, provider detail). Built a crawler prototype that normalizes Insight CME listings, generates embeddings, and syncs catalog rows into the legacy `Activity` table. Planner now refreshes from the catalog each rebuild, tests remain green, and docs capture the new retrieval architecture.
- **2025-10-24**: Rewired discovery so every activity flows through deep-fetch, OpenAI extraction, and Perplexity Sonar detail passes, guaranteeing modality, pricing tiers, eligibility, and scheduling metadata before committing updates. Normalized enrichment merges to refresh existing rows, surfaced “TBD” pricing in the planner instead of $0 placeholders, and h id redundant requirement badges. Full pytest suite passes, confirming no regressions in the refreshed pipeline.
- **2025-10-24**: Delivered the physician-facing three-column layout with the profile sidebar now housing requirements, policy status, and the activity log. Rebuilt the plan section and chat column to reflect the Tailwind mockups while maintaining all HTMX actions. Added first-class light/dark theming with a persistent toggle and updated CSS tokens so every surface shares the new visual language. Pytest remains green after the overhaul.
- **2025-10-23**: Enabled human-like plan editing by parsing natural-language removal/add requests, running targeted discovery for named activities, and only committing matches when titles align with the user’s phrasing. Added Perplexity-powered search ahead of Google CSE/OpenAI fallbacks, plus LLM-crafted query variants, so the assistant can find conferences like the APA Annual Meeting without losing context. Chat now announces when it’s searching, suppresses misleading credit-removal messages, and keeps the UI responsive while discovery runs in background threads.
